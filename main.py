...
def process_images(motion_gate: MotionGate) -> None:
...
def process_videos(motion_gate: MotionGate) -> None:
...
def process_camera(motion_gate: MotionGate) -> None:
    ...
    try:
        while True:
            ...
            if event and event.confidence >= CONFIDENCE_THRESHOLD:
                # In camera mode the event carries the confirmed frame
...
def draw_debug_overlay(frame, debug: Dict) -> None:
    ...
    def line(text: str, color=(255, 255, 255)):
        nonlocal y
    ...
    fps = debug.get("fps")
    line(f"FPS: {fps:.1f}" if fps else "FPS: -")
    ...
    if tracking_bbox:
        x, yb, w, h = map(int, tracking_bbox)
        if confirmed:
            label = debug.get("confirmed_label", "?")
            conf = float(debug.get("confirmed_conf", 0.0) or 0.0)
        ...
        cv2.rectangle(frame, (x, yb), (x + w, yb + h), color, 2)

        (tw, th), _ = cv2.getTextSize(text, cv2.FONT_HERSHEY_SIMPLEX, 0.5, 2)
        cv2.rectangle(frame, (x, yb - th - 6), (x + tw + 4, yb), color, -1)
        cv2.putText(frame, text, (x + 2, yb - 4), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 2)
